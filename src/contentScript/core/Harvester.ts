 ...
 */
import downloadButtonBSVG from '#assets/icons/twitter-download-b.svg'
...
const swapIconWith =
  ...
  <T extends HTMLElement>(icon: T) =>
    pipe(
      E.of(icon),
      E.flatMap(flow(getIconStyle, makeButtonIconWith(svgString), E.of)),
...
const makeButton =
  (mode: TweetMode, folder: 'a' | 'b', svgString: string) =>
  (article: HTMLElement) =>
    pipe(
      E.Do,
      E.bind('wipButton', () =>
      ...
      E.tap(({ wipButton }) =>
        pipe(wipButton, getIcon, E.flatMap(flow(swapIconWith(svgString))))
      ),
      E.bind('fullButton', ({ wipButton }) =>
        pipe(wipButton, wrapButton(mode, folder))
      ),
      E.tap(({ fullButton }) =>
...
export const makeHarvestButton = (article: HTMLElement) =>
  pipe(
    ...
    IOE.bind('buttonA', ctx =>
      pipe(
        makeButton(ctx.mode, 'a', downloadButtonSVG)(article),
        IOE.fromEither
      )
    ),
    IOE.bind('buttonB', ctx =>
      pipe(
        makeButton(ctx.mode, 'b', downloadButtonBSVG)(article),
        IOE.fromEither
      )
    ),
    IOE.tap(ctx => pipe(ctx.actionBar.appendChild(ctx.buttonA), IOE.of)),
    IOE.tap(ctx => pipe(ctx.actionBar.appendChild(ctx.buttonB), IOE.of)),
    IOE.tap(ctx => pipe(checkButtonStatus(ctx.buttonA), IOE.of)),
    IOE.tap(ctx => pipe(checkButtonStatus(ctx.buttonB), IOE.of)),
    IOE.map(() => 'ok')
  )

const wrapButton =
  (mode: TweetMode, folder: 'a' | 'b') => (button: HTMLElement) =>
    pipe(
      E.of({
        wrapper: createElementFromHTML(`
        <div class="harvester ${mode} harvester-folder-${folder}" data-testid="harvester-button-${folder}" data-harvest-ref="U2FsdGVkX18434vXoO+1oS21I0YQm8zFX6xy775AvdCpmSEOQHO9ns7wa518zD8t" data-folder="${folder}">
          <div aria-haspopup="true" aria-label="Media Harvest ${folder.toUpperCase()}" role="button" data-focusable="true" tabindex="0" \
...
const makeButtonIconWith = (svgString: string) => (svgStyle: string) => {
  const icon = createElementFromHTML(svgString)
